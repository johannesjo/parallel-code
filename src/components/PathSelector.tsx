...
export function PathSelector(props: PathSelectorProps) {
  ...
  // Fetch top-level entries once per project root. Nested paths are still
  // addable — users type them freely into the input.
  createEffect(() => {
    ...
    let cancelled = false;
    void invoke<Entry[]>(IPC.ListProjectEntries, { projectRoot: root })
      .then((result) => {
  ...
  const filtered = createMemo(() => {
    ...
    return entries().filter((e) => !added.has(e.name) && (!q || e.name.toLowerCase().includes(q)));
  });
