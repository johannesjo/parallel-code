      ...
      if (args.worktreePath) {
        ...
        try {
          ...
          if (fs.statSync(gitDir).isFile()) {
            ...
            infoDir = path.join(
              path.isAbsolute(realGitDir) ? realGitDir : path.resolve(args.worktreePath, realGitDir),
              'info',
            );
          } else {
        ...
        console.log('[MCP] Worktree .mcp.json written to:', worktreeMcpPath);
      }

      console.log('[MCP] Config written to:', configPath);
      console.log('[MCP] Server path:', mcpServerPath);
      console.log('[MCP] Remote URL:', serverUrl);
